(function ($, pubsub, util, elements, data, renderEngine) {
    var context = { resultCount: 0, notice: null, isActive: false, contextRequestId: null, isSelected: false, hasTried: undefined }, generateAjaxAddress = function () {
        ...
        elements.optionsHolder().find('.glimpse-clear-ajax').live('click', function () {
            pubsub.publish('trigger.shell.panel.clear.ajax');
        });
        panel.find('tbody a').live('click', function () {
            pubsub.publish('trigger.data.context.switch', { requestId: $(this).attr('data-requestId'), type: 'ajax' });
        });
        panel.find('.glimpse-head-message a').live('click', function () {
        ...
        var newPayload = args.newData, oldPayload = args.oldData, newId = newPayload.isAjax ? newPayload.parentRequestId : newPayload.requestId, oldId = oldPayload.isAjax ? oldPayload.parentRequestId : oldPayload.requestId;
        if (oldId != newId) {
        ...
        var panel = elements.panel('ajax'), detailPanel = panel.find('table');
        if (detailPanel.length == 0) {
            var detailData = [['Request URL', 'Method', 'Status Code', 'Duration', 'Date/Time', 'View']], detailMetadata = { layout: [[{ data: 0, key: true, width: '40%' }, { data: 1 }, { data: 2 }, { data: 3, width: '10%', className: 'mono', align: 'right' }, { data: 4, width: '20%' }, { data: 5, width: '100px' }]] };
        ...
        var panel = elements.panel('ajax'), detailBody = panel.find('tbody'), html = '';
        ...
        var panel = elements.panel('ajax'), row = panel.find('.selected');
        if (row.length > 0) {
        ...
        var link = elements.panel('ajax').find('.glimpse-ajax-link[data-requestId="' + args.requestId + '"]'), isDifferent = data.currentData().requestId != context.contextRequestId;
        ...
        var panel = elements.panel('ajax'), row = panel.find('tr[data-requestId="' + requestId + '"]');
