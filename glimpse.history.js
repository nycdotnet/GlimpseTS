(function ($, pubsub, util, settings, elements, data, renderEngine) {
    var context = { resultCount: 0, clientName: '', requestId: '', currentData: null, notice: null, isActive: false, isSelected: false, contextRequestId: undefined }, generateHistoryAddress = function () {
        ...
        elements.holder().find('.glimpse-clear-history').live('click', function () {
            pubsub.publish('trigger.shell.panel.clear.history');
        });
        panel.find('.glimpse-col-main tbody a').live('click', function () {
            pubsub.publish('trigger.data.context.switch', { requestId: $(this).attr('data-requestId'), type: 'history' });
        });
        panel.find('.glimpse-col-side tbody a').live('click', function () {
            selectSession($(this).attr('data-clientName'));
        });
        panel.find('.glimpse-col-main .glimpse-head-message a').live('click', function () {
        ...
        var panel = elements.panel('history'), masterPanel = panel.find('.glimpse-col-side');
        if (masterPanel.length == 0) {
            ...
            var detailPanel = panel.find('.glimpse-col-main'), masterData = [['Client', 'Count', 'View']], detailData = [['Request URL', 'Method', 'Status Code', 'Duration', 'Date/Time', 'Is Ajax', 'View']], detailMetadata = { layout: [[{ data: 0, key: true, width: '30%' }, { data: 1 }, { data: 2 }, { data: 3, width: '10%', className: 'mono', align: 'right' }, { data: 4, width: '20%' }, { data: 5, width: '10%' }, { data: 6, width: '100px' }]] };
        ...
        var panel = elements.panel('history'), detailBody = panel.find('.glimpse-col-main tbody'), html = '';
        ...
        var selected = settings.local('historyClient'), firstFound = '', panel = elements.panel('history'), masterBody = panel.find('.glimpse-col-side tbody');
        for (var recordName in result) {
            var masterRow = masterBody.find('a[data-clientName="' + util.jsEncode(recordName) + '"]').parents('tr:first'), rowCount = masterBody.find('tr').length;
        ...
        var panel = elements.panel('history'), detailPanel = panel.find('.glimpse-col-main'), row = detailPanel.find('.selected');
        if (row.length > 0) {
        ...
        var panel = elements.panel('history'), detailPanel = panel.find('.glimpse-col-main'), row = detailPanel.find('tr[data-requestId="' + requestId + '"]');
        ...
        var panel = elements.panel('history'), masterPanel = panel.find('.glimpse-col-side'), item = masterPanel.find('a[data-clientName="' + util.jsEncode(clientName) + '"]'), clientData = context.currentData[clientName];
        settings.local('historyClient', clientName), masterPanel.find('.selected').removeClass('selected');
