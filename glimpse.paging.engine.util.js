glimpse.paging.engine.util = (function ($, pubsub, data, elements, util, renderEngine) {
    var generatePagingAddress = function (pagerKey, pageIndex) {
        ...
        var pagingInfo = data.currentMetadata().plugins[key].pagingInfo, content = renderEngine.build(result, data.currentMetadata().plugins[key].structure), scope = renderMethod[method](elements.panel(key), content);
        ...
        append: function (panelItem, content) {
            var table = panelItem.find('> table'), nodes = $(content.substring(content.indexOf('>') + 1, content.lastIndexOf('<')))[1];
