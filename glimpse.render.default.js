(function (settings, pubsub, elements) {
    var readyOpen = function () {
        ...
        var isOpen = settings.local('isOpen');
        if (isOpen) {
            var current = settings.local('view'), tabElement = elements.tab(current), forced = current != null;
    ...
    }, currentTab = '', focusStart = function (key, isOpening) {
        if (key)
            currentTab = key;
        if (currentTab) {
    ...
    pubsub.subscribe('action.panel.showing', function (args) {
        focusStart(args.key, false);
    });
    pubsub.subscribe('action.shell.opening', function () {
        focusStart(null, true);
    });
    pubsub.subscribe('action.shell.closeing', function () {
        focusStop(true, false, false);
    });
    pubsub.subscribe('action.shell.minimizing', function () {
        focusStop(true, false, false);
    });
    pubsub.subscribe('action.panel.hiding', function () {
        focusStop(false, true, false);
    });
    window.onbeforeunload = function () {
        focusStop(false, false, true);
    };
