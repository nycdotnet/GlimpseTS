glimpse.render.engine = (function ($, pubsub) {
    var providers = {}, addition = function (scope, data, metadata, insertType, targetType) {
        var html = $('<div>' + build(data, metadata) + '</div>').find('.glimpse-row-holder:first > .glimpse-row'), rowHolder = scope.find('.glimpse-row-holder:first'), scopeTarget = rowHolder.find('> .glimpse-row:' + targetType);
