(function ($, util, engine, engineUtil) {
    var providers = engine._providers, build = function (data, level, forceFull, metadata, forceLimit) {
        ...
        for (var key in data) {
            var keyMetadata = engineUtil.keyMetadata(key, metadata), title = keyMetadata && keyMetadata.title ? util.preserveWhitespace(engineUtil.raw.process(util.processCasing(keyMetadata.title))) : engineUtil.raw.process(util.processCasing(key));
        ...
        var rowMax = 2, rowLength = util.lengthJson(data), rowLimit = (rowMax < rowLength ? rowMax : rowLength), i = 0, html = '<span class="start">{</span>';
        for (var key in data) {
