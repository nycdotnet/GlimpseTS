(function ($, util, engine, engineUtil, engineUtilTable) {
    var providers = engine._providers, buildFormatString = function (content, data, indexs, isHeadingRow) {
        for (var i = 0; i < indexs.length; i++) {
            var pattern = "\\\{\\\{" + indexs[i] + "\\\}\\\}", regex = new RegExp(pattern, "g"), value = isHeadingRow && !$.isNumeric(indexs[i]) ? indexs[i] : data[indexs[i]];
        ...
        var html = '', cellContent = '', cellClass = '', cellStyle = '', cellAttr = '', containsNestedData = $.isArray(metadataItem.data);
        if (containsNestedData) {
            ...
            if (metadataItem.engine && !isHeadingRow) {
                ...
                if (!isHeadingRow) {
                    if (metadataItem.pre) {
                        cellContent = '<span class="glimpse-soft">' + metadataItem.pre + '</span>' + cellContent;
                    }
                    if (metadataItem.post) {
                        cellContent = cellContent + '<span class="glimpse-soft">' + metadataItem.post + '</span>';
                    }
                }
            }
        }
        if (!isHeadingRow) {
            ...
            if (metadataItem.key === true) {
                cellClass += ' glimpse-cell-key';
            }
            if (metadataItem.isCode === true) {
                cellClass += ' glimpse-code';
            }
            if (metadataItem.className) {
                cellClass += ' ' + metadataItem.className;
            }
            if (metadataItem.codeType) {
                cellAttr += ' data-codeType="' + metadataItem.codeType + '"';
            }
            ;
        }
        if (cellClass) {
            cellAttr += ' class="' + cellClass + '"';
        }
        ;
        if (metadataItem.width) {
            cellStyle += 'width:' + metadataItem.width + ';';
        }
        ;
        if (metadataItem.align) {
            cellStyle += 'text-align:' + metadataItem.align + ';';
        }
        ;
        if (metadataItem.paddingLeft) {
            cellStyle += 'padding-left:' + metadataItem.paddingLeft + ';';
        }
        ;
        if (metadataItem.paddingRight) {
            cellStyle += 'padding-right:' + metadataItem.paddingRight + ';';
        }
        ;
        if (cellStyle) {
        ...
        ;
        if (metadataItem.span) {
            cellAttr += ' colspan="' + metadataItem.span + '"';
        }
        ;
        ...
        var html = '<table class="glimpse-row-holder">', layout = metadata.layout, factory = engineUtilTable.findFactory(data), headers = factory.getHeader(data);
