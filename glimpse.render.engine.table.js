(function ($, util, engine, engineUtil, engineUtilTable) {
    var providers = engine._providers, build = function (data, level, forceFull, metadata, forceLimit) {
        ...
        var html = '<table>', factory = engineUtilTable.findFactory(data), headers = factory.getHeader(data);
        ...
        var html = '<span class="start">[</span>', factory = engineUtilTable.findFactory(data), headers = factory.getHeader(data), startingIndex = factory.startingIndex(), columnMax = 3, columnLength = headers.length, columnLimit = columnMax < columnLength ? columnMax : columnLength, rowMax = 2 + startingIndex, rowLength = data.length - startingIndex, rowLimit = rowMax < rowLength ? rowMax : rowLength + startingIndex;
