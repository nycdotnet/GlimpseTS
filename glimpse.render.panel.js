(function ($, data, elements, pubsub, renderEngine) {
    var render = function (key, pluginData, pluginMetadata) {
        ...
        var panelHolder = elements.panelHolder(), permanent = pluginData.isPermanent ? ' glimpse-permanent' : '', html = '<div class="glimpse-panel glimpse-panelitem-' + key + permanent + '" data-glimpseKey="' + key + '"><div class="glimpse-panel-message">Loading data, please wait...</div></div>', panel = $(html).appendTo(panelHolder);
        ...
        var pluginData = data.currentData().data[options.key], pluginMetadata = data.currentMetadata().plugins[options.key], panel = elements.panel(options.key), currentSelection = elements.panelHolder().find('.glimpse-active');
