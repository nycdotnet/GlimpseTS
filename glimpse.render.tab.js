(function ($, data, elements, util, pubsub, settings) {
    var wireListeners = function () {
        ...
        if (!pluginData.suppressTab) {
            var disabled = pluginData.data == null ? ' glimpse-disabled' : '', permanent = pluginData.isPermanent ? ' glimpse-permanent' : '';
        ...
        for (var key in pluginDataSet) {
            var pluginData = pluginDataSet[key], itemHtml = generateHtmlItem(key, pluginData);
        ...
        var currentData = data.currentData(), tabInstanceHolder = elements.tabInstanceHolder(), tabPermanentHolder = elements.tabPermanentHolder();
        ...
        var tabHolder = elements.tabHolder(), tab = elements.tab(options.key);
        ...
        var key = args.key, payload = args.payload, itemHtml = generateHtmlItem(key, payload);
