(function ($, pubsub, elements, data, util) {
    var wireListeners = function () {
        elements.barHolder().find('.glimpse-icon').click(function () {
            pubsub.publish('trigger.tab.select.info', { key: 'info' });
        });
    }, buildHelp = function (options) {
        var info = data.currentMetadata().plugins[options.key], url = info && info.documentationUri;
