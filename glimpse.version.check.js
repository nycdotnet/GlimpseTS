glimpse.versionCheck = (function ($, pubsub, settings, elements, data, util) {
    var retrieveStamp = function () {
        ...
        var hasNewerVersion = settings.local('hasNewerVersion'), versionCheckUri = settings.local('versionCheckUri') || '', currentHash = settings.local('hash');
        if (hasNewerVersion && versionCheckUri.indexOf('hash=' + currentHash) > -1)
            elements.holder().find('.glimpse-meta-update').show();
    }, ready = function () {
        var nextChecked = settings.local('nextCheckedVersionTime'), now = new Date();
        tryShow();
        if (nextChecked) {
            var nextCheckedTicks = parseInt(nextChecked), currentTimeTicks = now.getTime();
